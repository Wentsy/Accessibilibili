...
class RenderEditable extends base.RenderEditable {
  ...
  String? _spokenEmoteName(RichTextItem item) {
    ...
    // Keep Bilibili's bracketed token for native attachment speech, e.g.
    // "[doge]". VoiceOver will add the attachment role itself, so never append
    // "表情". If a source name already contains that suffix, remove only the
    // suffix and preserve any surrounding brackets.
    if (name.length >= 2 && name.startsWith('[') && name.endsWith(']')) {
      var inner = name.substring(1, name.length - 1).trim();
