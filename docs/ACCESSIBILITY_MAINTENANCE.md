...
- **目前已知完整實機驗證過的無障礙基準：`ffa4a3711a2a3ca4a09cf79fa3f064d94940092c`**（2026-09-23，2.1.3+5722）
  - 在 5715 固定線路緩衝版基底之上一併驗證通過：一般影片開播網路類型確認與取得影片網址並行、直播重連去重（同時只允許一個重連、恢復／暫停／換台取消舊重試、緩衝耗盡約補 2 秒續播）、直播貼圖焦點穩定、彈幕輸入框改用 iOS 原生編輯器（表情朗讀名稱）、外層評論頂端三指下滑重新整理／底端三指上滑載入更多、直播彈幕列表頂端下滑刷新／底端上滑更新到最新。
  - 未來同步上游時，這批直播重連與緩衝、外層評論與直播彈幕三指手勢、直播貼圖焦點、彈幕 iOS 原生編輯器行為都不可回退。

- **目前已知完整實機驗證過的無障礙基準：`fc8a520ce92833c19b43741c387a9cb45e5889ec`**（2026-09-11）
  ...
  - 2026-09-10 實機驗證完成：首頁推薦、動態、觀看紀錄的三指邊界刷新／載入更多，以及刷新與載入狀態朗讀。方向固定為三指下滑往回翻、頂端再下滑刷新；三指上滑往下一頁、底端再上滑載入更多。
  - 2026-09-11 實機驗證完成：大量評論及樓中樓連續閱讀可跨多次分頁讀到底；載入邊界不再卡住，不需先往左退再往右滑；修復套用至所有評論入口。無新增重試按鈕，保留真正結尾停止與 VoiceOver 焦點位置。
  - 2026-09-11 實機驗證完成：原生評論最後一則查找效能優化後，左右滑評論比 5669 順暢，採用 `fc8a520` 作為目前穩定基準。
...
固定底部發表列是後續待驗證布局，詳見 `docs/COMPOSER_DOCK.md`；手機測試通過前，
不應把它描述為已完成實機驗證的基準。
...
- 首頁推薦、動態、觀看紀錄的方向固定：三指下滑向列表頂端，頂端再次下滑重新整理；三指上滑向列表底端，底端再次上滑載入更多。不要用相反的語意映射。
...
- [ ] 首頁／動態／觀看紀錄：三指下滑往回翻，頂端再下滑刷新；三指上滑往下一頁，底端再上滑載入更多，方向不能反。
- [ ] 上述刷新／載入操作會朗讀進行中、完成／失敗或沒有更多內容。
- [ ] 翻頁或自動載入更多後不跳回第一項。
- [ ] Web 推薦影片可朗讀發布時間。
...
- [ ] 在 App 前景先暫停，再回主畫面與鎖屏；兩處 VoiceOver 雙指雙擊都能恢復播放。
- [ ] 動態卡片的「造訪使用者」可開啟該動態作者的 UP 主頁，不出現空白頁。
