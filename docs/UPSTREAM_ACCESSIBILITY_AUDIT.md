...
## 2026-09-23 稽核更新（2.1.3+5722 新增差異）

本批（5718–5722）新增以下 Accessibilibili 相對官方版的無障礙／播放差異，同步上游時一併保留：

- **外層評論三指手勢（影片與動態）**：頂端三指下滑＝重新整理並朗讀狀態；底端三指上滑＝載入更多並朗讀完成／失敗／「沒有更多評論」。樓中樓不動，連續朗讀的自動載入保持安靜。上游若改動 `lib/pages/video/reply/`、`lib/pages/dynamics/` 的外層評論列表或三指處理，需重新驗收。
- **直播彈幕列表三指手勢**：滑到頂端後三指下滑＝重新整理並留在頂端；滑到底端後三指上滑＝更新到最新彈幕，不必再移到高能觀眾或發送欄。上游若改動直播聊天室捲動或手勢，需重新驗收。
- **直播貼圖焦點固定識別**：焦點移動時不做額外捲動，降低選錯貼圖。
- **彈幕輸入框改用 iOS 原生編輯器**：表情可朗讀名稱（與評論區相同），不依賴 Flutter 自繪編輯器。
- **直播重連去重**：同時只允許一個重連；恢復、暫停或換台時取消舊重試；緩衝耗盡後約補 2 秒再續播。上游若改動直播重連／緩衝邏輯，需保留此行為。
- **一般影片開播並行**：確認網路類型與取得影片網址同時進行，固定線路與緩衝參數不變。
