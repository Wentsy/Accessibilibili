...
- 使用者選定的基底：**2.1.3+5708**。
- 已驗證 App 程式碼基準：`263827f257f69da7ebf606ca9044bbeb1b625105`。
- 使用者後續比較確認，5708 在穩定網路下開播及進入樓中樓的載入感受較快，已自行還原使用。
- 撤回將自動選線版／`2fbda93d` 視為全面較佳穩定基準的結論；先前一天的背景播放改善回報保留為歷史觀察，不代表所有場景均通過。
- 樓中樓載入變慢的原因未確定，不再直接判定為網路影響。

### 目前 main 的候選調整（尚待編譯與實機確認）

以 5708 程式碼為底，移除自動選線程式、設定入口、探測與換線計時器，恢復 5708 的手動 CDN 處理。保留 5708 的 60 秒／32 MiB 預讀預算與快速起播，只將自動緩衝耗盡後的恢復目標由 3 秒提高到 8 秒，並移植有時限的 iOS 背景載入保護。

不移植 120 秒／64 MiB 預讀加倍、10 秒網路逾時或自動 CDN 重試。評論程式維持 5708。
`main` 的候選修改不等於已驗證穩定版，須由 Hermes 編譯及使用者實測後再更新基準。
