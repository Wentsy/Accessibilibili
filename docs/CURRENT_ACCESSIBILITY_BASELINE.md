...
## 2026-09-22 播放緩衝與鎖屏恢復穩定基準

基於 commit `2fbda93dca07a4771a40b36e2f770d92c47243ca`，使用者已連續實機觀察約一天，確認：

- 背景播放可正常持續，鎖定螢幕後的播放與載入比前一版穩定，卡頓明顯減少。
- 弱網下的影片預讀與分段銜接改善，長影片播放途中不再容易因載入停住。
- 本次緩衝修正納入目前穩定基準，後續更新應保留預讀儲備、緩衝恢復及鎖屏背景載入保護。
- 評論區進入樓中樓時的載入速度，本次觀察較像網路影響；目前沒有足夠證據判定為程式 regression，先不納入修改項目。

本次由使用者於 2026-09-22 回報實機觀察穩定。後續若評論載入卡頓在不同網路環境仍可穩定重現，再另行檢查評論分頁與載入併發。
