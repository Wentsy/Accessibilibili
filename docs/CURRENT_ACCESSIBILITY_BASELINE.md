...
- 使用者選定的基底：**2.1.3+5722**。
- 已驗證 App 程式碼基準：`ffa4a3711a2a3ca4a09cf79fa3f064d94940092c`（2026-09-23 希希實機驗證通過）。
- 5722 以 5715 固定線路緩衝版（`efdab3d8`）為底，疊加本批開播與直播優化：
  - `7b314a1a`（5718）：一般影片開播時「確認網路類型」與「取得影片網址」並行進行，減少前後等待。
  - `eadd554e`（5719）：直播同時只允許一個重連；恢復、暫停或換台時取消舊重試；緩衝耗盡後約補 2 秒續播、起播不強制等滿。
  - `f7f6fc6`（5720）：直播貼圖移除焦點移動時的額外捲動＋固定識別；彈幕輸入框改用 iOS 原生編輯器（表情可朗讀名稱）；外層評論頂端三指下滑重新整理。
  - `44cb99d`（5721）：影片與動態外層評論統一支援頂端三指下滑重新整理、底端三指上滑載入更多，並朗讀進行中／完成／失敗／「沒有更多評論」狀態；樓中樓與表情不動，連續朗讀的自動載入保持安靜。
  - `ffa4a37`（5722）：直播彈幕列表直接支援三指手勢——滑到頂端後三指下滑重新整理並留在頂端；滑到底端後三指上滑更新到最新彈幕。
- 固定線路原則不變：不啟用自動選線、背景測速或頻繁換線；後續緩衝與開播優化都以維持希希選定的播放線路為前提。
...
## 2026-09-23：2.1.3+5722 開播並行＋直播手勢版正式穩定

- 使用者實際測試版本：2.1.3+5722。
- 已驗證程式碼基準：`ffa4a3711a2a3ca4a09cf79fa3f064d94940092c`。
- 基底為 5715 固定線路緩衝版（`efdab3d8`），實際測試與穩定版號為 2.1.3+5722。
- 希希 2026-09-23 實機驗證通過；直播短暫網路波動不再連續重開、手動暫停不會被舊重試打斷，直播彈幕與外層評論的三指手勢可用。
- 普遍影片開播等待縮短（網路類型確認與影片網址取得並行）；固定線路、預讀、鎖屏保護與評論程式維持 5715 行為。
- 自動選線方案仍不納入穩定基準；本批所有優化都以維持使用者選定的線路為前提。

## 2026-09-22：2.1.3+5715 固定線路緩衝版正式穩定

- 使用者實際測試版本：2.1.3+5715。
- 已驗證程式碼基準：efdab3d88fd8f2811a40b36e2f770d92c47243ca。
- 基底為使用者選定的 2.1.3+5708（263827f257f69da7ebf606ca9044bbeb1b625105），實際測試與穩定版號為 2.1.3+5715。
- 使用者實測確認：網路穩定時，開播與進入樓中樓的載入速度較快；弱網時緩衝改善仍保留。
- 保留連續預讀與分塊銜接，盡量在前一段載入後立即準備下一段；不加入開播前測速或等待。
- 自動選線方案不納入穩定基準。實測顯示穩定網路下背景頻繁換線可能增加等待、反而影響順暢度；後續除非另有明確測試，不重新啟用。
- 評論區進入樓中樓的載入速度先以本版觀察，沒有另改評論程式。
